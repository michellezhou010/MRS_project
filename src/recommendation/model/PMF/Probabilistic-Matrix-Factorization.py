...
class PMF(object):
    ...
    def fit(self, train_vec, val_vec):
        ...
        # num_user = len(np.unique(train[:, 0]))+1
        # num_item = len(np.unique(train[:, 1]))+1
        incremental = False
        ...
        while self.epoch < self.maxepoch:
            ...
            for batch in range(self.num_batches):
                ...
                batch_invID = np.array(train_vec[shuffled_order[batch_idx], 0]).astype('int64')
                batch_comID = np.array(train_vec[shuffled_order[batch_idx], 1]).astype('int64')
                # pdb.set_trace()
